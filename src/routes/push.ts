...
router.post('/', (req, res) => {
  ...
  Promise.all(req.body.receivers.map((userId: string) => {
    DeviceService.getDevices(req.body.platformId, mongoose.Types.ObjectId(userId))
      .then((devices: any) => {
          ...
          return pushService.send(req.body.platformId, message)
            ...
            .catch((e: FirebaseError) => {
              if (e.code === 'messaging/registration-token-not-registered') {
                pushService.removeToken(req.body.platformId, userId, device);
              }
              winston.error(e);
