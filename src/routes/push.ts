...
const PromiseAny = function (promises: Promise<any>[]) {
...
router.post('/', (req, res) => {
  ...
  const templatingService: TemplatingService = new TemplatingService(req.body.platform, req.body.template,
    req.body.payload, req.body.languagePayloads);

  const push: firebaseMessaging.Message = {
    token: req.body.token,
    data: Object.assign({},req.body.data,{_id:Utils.guid()}),
  ...
  Promise.all(req.body.receivers.map((user: UserResource) => {
    DeviceService.getDevices(req.body.platform, user._id)
      .then((devices: any) => {
        ...
        return PromiseAny(devices.map((device: string) => {
          //const message = Object.assign({}, push, { token: device });
          const pushMessage = templatingService.createPushMessage(user, device);
          //pushService.send(req.body.platform, pushMessage);
          return pushService.send(req.body.platform, pushMessage)
            .then((response: any) => {
              winston.info(response);
            ...
            .catch((e: FirebaseError) => {
              if (e.code === 'messaging/registration-token-not-registered') {
                pushService.removeToken(req.body.platform, user._id, device);
              }
              winston.error(e);
